...
class Route {
        ...
        if(is_array($values)){
            foreach($values AS $index=>$value){
                switch($type){
                    ...
                    case 'file' : $_FILES[$index] = $value; break;
                }
            }
            return true;
        }
        ...
        if(is_array($values)){
            foreach($values AS $index=>$value){
                switch($type){
                    ...
                    case 'file' : unset($_FILES[$index]); break;
                }
            }
        }
    }
	...
	public static function isSiteRequest(){
		$referer = explode('/',str_replace(array('http://','https://'),'',$_SERVER['HTTP_REFERER']));
        $dominio = explode('/',str_replace(array('http://','https://'),'',$_SERVER['HTTP_HOST']));
		$isDominio = reset($dominio);
		$isReferer = reset($referer);
